...
"""
BigRedDog DWAQ Sim2Sim 脚本 (带步态相位版本)
=====================================

将在 Isaac Lab 中训练的 BigRedDog 机器人 DWAQ 策略迁移到 MuJoCo 仿真环境中运行。
...
使用方法：
---------
python legged_lab/scripts/sim2sim_bigreddog_dwaq.py --checkpoint=/home/csl/Desktop/U1/G1DWAQ_Lab/TienKung-Lab/logs/bigreddog_dwaq/2026-03-02_13-42-54/model_9999.pt
...
class BigRedDogDwaqSim2SimCfg:
    """BigRedDog DWAQ Sim2Sim 配置类 - 无步态版本"""
...
class BigRedDogDwaqMujocoRunner:
    """
    BigRedDog DWAQ Sim2Sim 运行器 (带步态相位版本)
    
    加载 ActorCritic_DWAQ 策略和 MuJoCo 模型，运行实时仿真控制。
    步态相位用于实现稳定的两足交替行走。
    """

    def __init__(self, cfg: BigRedDogDwaqSim2SimCfg, checkpoint_path: str, model_path: str):
        self.cfg = cfg
    ...
    def init_variables(self) -> None:
        ...
        # 默认关节位置 (MuJoCo 顺序) - 与 BigRedDog_CFG init_state.joint_pos 一致
...
def main():
    ...
    MJCF_DIR = os.path.join(LEGGED_LAB_ROOT, "legged_lab/assets/unitree/bigreddog")
    LOGS_DIR = os.path.join(LEGGED_LAB_ROOT, "logs")
    
    # 尝试查找最新的 bigreddog_dwaq checkpoint
    # default_checkpoint = find_latest_checkpoint(LOGS_DIR, "bigreddog_dwaq")
    # if default_checkpoint is None:
    #     # 回退到硬编码路径
    #     default_checkpoint = os.path.join(LOGS_DIR, "bigreddog_dwaq/2026-01-15_11-21-04/model_10000.pt")

    default_checkpoint = os.path.join(LOGS_DIR, "bigreddog_dwaq/2026-01-16_00-46-00/model_9999.pt")
    ...
    parser = argparse.ArgumentParser(
        description="BigRedDog DWAQ Sim2Sim - 盲走策略 (无步态版本)",
    ...
    if not os.path.isfile(args.checkpoint):
        ...
        print(f"       python {sys.argv[0]} --checkpoint logs/bigreddog_dwaq/2026-xx-xx/model_10000.pt")
        sys.exit(1)
    ...
    cfg = BigRedDogDwaqSim2SimCfg()
    cfg.sim.sim_duration = args.duration
    
    runner = BigRedDogDwaqMujocoRunner(
        cfg=cfg,
