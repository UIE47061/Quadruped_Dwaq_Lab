...
from legged_lab.assets.unitree import Little_White_CFG
...
@configclass
class LittleWhiteDwaqRewardCfg(RewardCfg):
    ...
    # 輕微懲罰大腿與小腿偏離預設姿勢，防止腿部長時間不自然伸展（如往後伸直拖行）
...
@configclass
class LittleWhiteDwaqEnvCfg(BaseEnvCfg):
    """GO2 DWAQ environment configuration for blind walking with VAE."""
    
    reward = LittleWhiteDwaqRewardCfg()
    ...
    def __post_init__(self):
        ...
        self.scene.robot = Little_White_CFG
...
@configclass  
class LittleWhiteDwaqAgentCfg(BaseAgentCfg):
    """GO2 DWAQ agent configuration."""
    experiment_name: str = "little_white_dwaq"
    wandb_project: str = "little_white_dwaq"
