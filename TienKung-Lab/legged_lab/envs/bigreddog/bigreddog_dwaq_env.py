...
"""
BigRedDog DWAQ Environment - Blind Walking with Variational Autoencoder

This environment implements DWAQ (Deep Variational Autoencoder for Walking) training
for the BigRedDog robot. It follows the Isaac Lab interface style where step() returns
...
from legged_lab.envs.bigreddog.bigreddog_dwaq_config import BigRedDogDwaqEnvCfg

from legged_lab.utils.env_utils.scene import SceneCfg
from rsl_rl.env import VecEnv


class BigRedDogDwaqEnv(VecEnv):
    """
    BigRedDog DWAQ environment for blind walking with β-VAE.
    ...
    def __init__(
        self,
        cfg: BigRedDogDwaqEnvCfg,
        headless: bool,
    ):
        self.cfg: BigRedDogDwaqEnvCfg
    ...
    def init_buffers(self):
        ...
        self.num_feet = len(self.feet_cfg.body_ids)
        self.forward_obstacle_height = torch.zeros(self.num_envs, self.num_feet, device=self.device)
        self.terrain_height_at_feet = torch.zeros(self.num_envs, self.num_feet, device=self.device)
    ...
    def compute_current_observations(self):
        ...
        # Build critic obs with privileged information (same as BigRedDog_rgb_env)
    ...
    def _compute_forward_obstacle_height(self):
        ...
        # ========== 第一步：计算每只脚正下方的地形高度 ==========
        terrain_under_feet = torch.zeros((self.num_envs, self.num_feet), device=self.device)
        ...
        # ========== 第二步：确定参考地形高度 ==========
        # 对于四足机器人，简化处理：直接使用每只脚下方的地形作为参考
        self.terrain_height_at_feet = terrain_under_feet
